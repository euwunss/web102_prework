...
function addGamesToPage(games) {
  ...
  for (const game of games) {
    ...
    const gameInfo = `
        <img class="game-img" alt="Game Logo" src=${game.img} />
        <h3>${game.name}</h3>
...
const totalContributions = GAMES_JSON.reduce((acc, e) => acc + e.backers, 0);
...
const totalRaised = GAMES_JSON.reduce((acc, e) => acc + e.pledged, 0);
...
const descriptionStr = `A total of $${totalRaised.toLocaleString(
  "en-US"
)} has been raised for ${gamesNumber} games. Currently, ${numUnfundedGames} ${
...
descriptionPar.innerHTML = descriptionStr;
